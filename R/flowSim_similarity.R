...
#' @param list_all_input_dfs Output generated by the import_all_dfs function
...
#' Generate similarity scores all files first version
#' 
#' function to generate the similarity scores for all the files within the input directory. First version (for small datasets only,N<500).
...
#' @param thr_score Threshold for filtering only the most significant similarity score, it ranges from 0 to 1. Default to 0.9
...
get_similarity_all_plots<-function(n_samples=NULL,path_dir,thr_score=0.9,
                                   n_cores=1){
  start<-Sys.time()
...
#' Generate similarity scores all files second version
#' 
#' function to generate the similarity scores for all the files within the input directory. Second version, for big datasets.
...
#' otherwise it needs to be defined. As the value increases, less memory and time is required.
...
get_similarity_all_plots_v2<-function(n_samples=NULL,path_dir,thr_score=0.9,
                                      ...
                                      progress_bar=T,size_points_tsne=2){
  set.seed(40)
  start<-Sys.time()
  #---------- Features df -------------------
  ...
  list_results<-batches_analysis(all_groups = all_groups,n_cores = n_cores,final_df = final_df,
                                 all_plots_names = all_plots_names,paths_all_plots = paths_all_plots,
                                 progress_bar = progress_bar,thr_score = thr_score)
...
#' batches_analysis
#' 
#' function to analyze each batch. Internal function.
#' @param all_groups All batches names to analyze
#' @param n_cores Number to use
#' @param final_df Dataframe of features reduced
#' @param all_plots_names All plots names under analysis
#' @param paths_all_plots Path of all plots under analysis
...
batches_analysis<-function(all_groups,n_cores,final_df,all_plots_names,
                           paths_all_plots,thr_score,progress_bar=T){
  if(progress_bar==T){
    ...
    list_results<-foreach(i=1:length(all_groups),.options.snow=opts) %dopar% {
      ...
      check_plots<-all_plots_names %in% samples_name_current_group
      inds<-which(check_plots==T)
      paths_plots_current_group<-paths_all_plots[inds]
      #----- import list dfs current samples
      list_all_input_dfs_current_group<-import_all_dfs(paths_plots = paths_plots_current_group,n_cores = n_cores)
      #------------ Get all plot names ---------------
      # plot names will be  the nodes of our network
